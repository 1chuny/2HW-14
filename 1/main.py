...
def create_access_token(data: dict, expires_delta: timedelta = None) -> str:
    ...
    Args:
        ...
        expires_delta (timedelta, optional): The expiration time for the token. Defaults to 15 minutes if not provided.
...
def create_contact(contact: schemas.ContactCreate, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_user)):
    ...
    Args:
        contact (schemas.ContactCreate): The data required to create a new contact.
        db (Session): The database session.
...
def update_contact(contact_id: int, contact: schemas.ContactCreate, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_user)):
    ...
    Args:
        ...
        contact (schemas.ContactCreate): The updated contact data.
        db (Session): The database session.
        current_user (schemas.User): The currently authenticated user.
...
async def upload_avatar(file: UploadFile, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_user)):
    """
    Uploads and updates the avatar for the authenticated user.

    Args:
        file (UploadFile): The file to upload as an avatar.
        db (Session): The database session.
        current_user (schemas.User): The currently authenticated user.
